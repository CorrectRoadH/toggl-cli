...
fn live_cli_round_trip_covers_list_create_mutate_and_cleanup() {
    ...
    wait_for("created task did not move under renamed project", || {
        let tasks = list_tasks();
        tasks
            .iter()
            .any(|item| item.id == created_task.id && item.project.name == renamed_project_name)
            .then_some(())
    });
    run_toggl(&[
        "edit",
    ...
    assert!(
        tasks_after_delete.iter().all(|item| {
            item.project.id != renamed_project.id && item.project.name != renamed_project_name
        }),
